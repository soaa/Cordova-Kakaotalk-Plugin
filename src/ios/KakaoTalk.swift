...
@objc(KakaoTalk) class KakaoTalk: CDVPlugin {
            ...
            if(session.isOpen()) {
                ...
                KOSessionTask.meTask(completionHandler: { (response, error) in
                    if let user = response as? KOUser {
                        ...
                        NSLog("no response?")
                        result = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "invalid response")
                    }
                })
            } else {
                // failed
    ...
    @objc(link:) func link(command: CDVInvokedUrlCommand) {
        ...
        if (template != nil) {
            ...
            result = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "parameter error")
        }
